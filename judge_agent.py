    ...
    def evaluate(self, query: str, results: Sequence[WrapResult], history: str, iteration: int) -> Dict[str, Any]:
        ...
        if require_citation:
            ...
            if jr.is_sufficient and authoritative_exists and no_contradiction:
                ...
                # fallback to score threshold (Relaxed to 40%)
                if score_pct >= 40:
                    passed = True
                else:
                    deterministic_reasons.append(f"Aggregate score too low: {score_pct}% (needs >=40 for topical queries).")
...
class EvaluateAgent:
    ...
    def run(self, max_iterations: int = 6) -> Dict[str, Any]:
        iteration = 1
        final = None
