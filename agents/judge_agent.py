...
def get_domain_type(url: Optional[str]) -> str:
    ...
    # ✅ ADD: Indian and academic legal sources
    if "indiankanoon" in hostname:
        return "indiankanoon"
    if "devgan" in hostname:
        return "devgan"
    if "semanticscholar" in hostname or "semantic" in hostname:
        return "semantic"
...
class WrapResult:
    def __init__(self, d: Dict[str, Any], summary_limit: int = 1500):
        ...
        self.summary_for_judge: str = self.full_content #self.build_summary(self.full_content, summary_limit)
    ...
    def build_metadata(self) -> Dict[str, Any]:
        ...
        HIGH_AUTHORITY = {
            "gov", "edu", "court_decision",
            "indiankanoon", "devgan", "semantic"
        }
        basic["is_authoritative"] = domain_type in HIGH_AUTHORITY
...
Be PRAGMATIC: If the retrieved content answers the core of the query, mark it as SUFFICIENT even if minor details are missing.
Do not reject relevant results just because they are not from a government domain.
Use the provided METADATA fields to perform these checks:
  1) SOURCE QUALITY CHECK:
     - Use 'domain_type' and 'is_authoritative' to mark authority (gov/edu/court_decision/indiankanoon/devgan/semantic => high authority).
...
"""
    ...
    def evaluate(self, query: str, results: Sequence[WrapResult], history: str, iteration: int) -> Dict[str, Any]:
        ...
        # Scoring weights (adjusted for better recall)
        score = 0
        # Reduced authority weight (was 45) to allow good 3rd party analysis
        ...
        max_possible = 30 + 25 + 25 + 20 + 25  # =125
        score_pct = int((score / max_possible) * 100) if max_possible > 0 else 0

        # Decision thresholds (Relaxed):
        # - If the query asked for a canonical reporter citation:
        #     require citation_present OR score_pct >= 65 (was 85)
        # - Otherwise (topical queries), accept if score_pct >= 40 (was 55)
        passed = False
        ...
        if require_citation:
            ...
            # Relaxed threshold to 65%
            if score_pct >= 65 or (citation_present and authoritative_exists and no_contradiction):
                passed = True
        else:
            # topical query path
            # if the LLM says sufficient and there are no contradictions and authoritative result exists -> accept
            # ✅ Pragmatic topical acceptance (matches prompt)
            if jr.is_sufficient and no_contradiction:
                passed = True
            else:
                if score_pct >= 40:
                    passed = True
                    ...
                    deterministic_reasons.append(
                        f"Aggregate score too low: {score_pct}% (needs >=40 for topical queries)."
                    )
        ...
        # ✅ Only report authority failure if it actually caused rejection
        if not authoritative_exists and not passed:
            deterministic_reasons.append(
                "No authoritative result found (gov/edu/court_decision)."
            )
...
# ----------------------------
class EvaluateAgent:
    def __init__(self, query: str, judge_key_env: str = "GOOGLE_API_KEY"):
        ...
        raw = run_all(query, True)
        # STORE RAW JSON results for the initial query
    ...
    def _pre_refine_for_citation_if_needed(self, original_query: str) -> None:
        ...
        try:
            ...
            # STORE RAW JSON results for the refinement
            self.raw_results = raw
    ...
    def run(self, max_iterations: int = 5) -> Dict[str, Any]:
        ...
        while iteration <= max_iterations:
            ...
            raw = run_all(new_query, False)
            # STORE RAW JSON results for each refinement iteration
            self.raw_results = raw
            self.results = self._normalize_raw_results(raw)
            iteration += 1
        ...
        return {
            ...
            # RETURN the exact raw JSON produced by run_all() for the final query
